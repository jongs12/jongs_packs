$execute positioned ~-0.5 ~-0.5 ~-0.5 as @e[dx=0,nbt=!{UUID:$(UUID)},type=!#jongs:ranged_weapons/infinite_health,predicate=jongs:ranged_weapons/not_invulnerable] run return run damage @s 8 minecraft:sweet_berry_bush by @p[scores={jongs_ranged_weapons_loaded=1}]
