...
execute store result score 현재값 jongs_sword_dash_click run data get entity @s Item.components."minecraft:custom_data".jongs[{purpose:"sword_dash"}].levels.distance
execute store result score 추가값 jongs_sword_dash_click run data get entity @n[type=item,nbt={Item:{id:"minecraft:wind_charge"}}] Item.count 5
