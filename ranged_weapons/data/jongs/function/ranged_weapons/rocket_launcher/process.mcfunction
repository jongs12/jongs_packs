...
$execute positioned ^ ^ ^1.5 store success score @s jongs.ranged_weapons.success positioned ~-0.5 ~-0.5 ~-0.5 as @e[dx=0,nbt=!{UUID:$(shooter)},type=!#jongs:ranged_weapons/infinite_health] run damage @s 50 minecraft:player_explosion by @p[nbt={UUID:$(shooter)}]
$execute if score @s jongs.ranged_weapons.success matches 1 run return run function jongs:ranged_weapons/rocket_launcher/explosion {"UUID":$(shooter)}
