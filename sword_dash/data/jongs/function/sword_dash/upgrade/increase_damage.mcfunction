execute store result score #현재값 jongs.sword_dash.click run data get entity @s Item.components."minecraft:custom_data".jongs[{purpose:"sword_dash"}].levels.damage
execute store result score #추가값 jongs.sword_dash.click run data get entity @n[predicate=jongs:sword_dash/is_fire_charge] Item.count 2
...
kill @n[predicate=jongs:sword_dash/is_fire_charge]
