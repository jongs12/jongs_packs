...
execute unless items entity @s weapon.mainhand *[minecraft:damage~{durability:{min:1}}] if score @s jongs.ranged_weapons.reload matches 7001..7999 run return run scoreboard players remove @s jongs.ranged_weapons.reload 1
execute unless items entity @s weapon.mainhand *[minecraft:damage~{durability:{min:1}}] run return run scoreboard players set @s jongs.ranged_weapons.reload 7051
