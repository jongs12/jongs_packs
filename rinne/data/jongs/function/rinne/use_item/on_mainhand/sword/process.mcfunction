...
execute positioned ~-1 ~-1 ~-1 as @e[dx=1,dy=1,dz=1,tag=!jongs.rinne.shooter,type=!#jongs:rinne/infinite_health] run damage @s 5 minecraft:player_attack by @e[type=minecraft:item_display,tag=jongs.rinne.shooting,limit=1] from @a[tag=jongs.rinne.shooter,limit=1]
