...
$execute unless entity @s[gamemode=creative] unless entity @s[level=$(level_need)..] run return run title @s actionbar {"translate":"jongs.ranged_weapons.low_level","color":"#C0C0C0","shadow_color":[0,0,0,1],"with":["$(level_need)"]}
...
return run title @s actionbar {"translate":"jongs.ranged_weapons.reloading","shadow_color":[0,0,0,1]}