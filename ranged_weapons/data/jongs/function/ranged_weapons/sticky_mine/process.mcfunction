...
$execute if score @s jongs.ranged_weapons.distance matches 0..299 store success score @s jongs.ranged_weapons.success positioned ~-1 ~-1 ~-1 as @e[dx=1,dy=1,dz=1,nbt=!{UUID:$(shooter)},type=!#jongs:ranged_weapons/no_explosion_target] run damage @s 30 minecraft:arrow by @p[nbt={UUID:$(shooter)}]
