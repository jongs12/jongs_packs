$execute positioned ~-3 ~-3 ~-3 as @e[dx=5,dy=5,dz=5,nbt=!{UUID:$(UUID)},tag=!jongs_sword_dash_hit] run tag @s add jongs_sword_dash_hit
...
execute if score @s jongs_sword_dash_distance matches ..-1 positioned ^ ^ ^0.1 run return run function jongs:sword_dash/dashing/process with entity @s
